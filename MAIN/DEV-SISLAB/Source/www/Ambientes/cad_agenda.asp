...
s = "SELECT a.*, crt.NM_CRT From Ambientes a INNER JOIN CentroReferencia crt ON a.ID_CRT = crt.ID_CRT " & _
    "WHERE AMB_USADOPORAG = 0 AND AMB_MODULO = " & Application("SISLAB_ID_APLICACAO_SISLAB") & " order by AMB_Nome asc;"
call Env.RecordSet( true, objRS, s)
If Not objRS.EOF Then
	objRS.MoveFirst %>
	 ...
	 <% do while not objRS.EOF %>
	    	    <option value=<%=objRS("AMB_ID")%>><%=objRS("AMB_Nome")%> - [<%=objRS("NM_CRT")%>]</option> 
	<%objRS.movenext%>
	<%loop%>
    		</select>
<%  end if %>
