...
<!--#in clude file="../includes/controlesHTML.asp" -->
...
<%
Dim ssql, rec, recFab, conn
Dim eq_id, categoria, descricao, instrumental, fab_id, fab_id_old, mod_id
	...
	function atualizarInventario(eq_id, limpar){
		...
		maxAjaxObj.get();
			...
			<td class="texto" align="right" colspan="2">
				<table bgcolor="#CCCCCC"  class="texto" cellpadding="2" cellspacing="2" border="0">
				<tr>
					<td>
						Último Inventário:<br>&nbsp;&nbsp;&nbsp;<input type="text" id="dtUltInventario" name="dtUltInventario" size="19" class="form" value="<%=DtUltInventario%>">
<%				'If session("status") = PERFIL_RAT Then %>
<%				If session("status") = PERFIL_ADM Then %>
						&nbsp;&nbsp;<input type="button" value=" Marcar " class="texto" onClick="javascript:atualizarInventario(<%=eq_id%>, false);">
						&nbsp;&nbsp;<input type="button" value="Desmarcar" class="texto" onClick="javascript:atualizarInventario(<%=eq_id%>, true);">
<%				End If %>
					</td>
				</tr>
				<tr><td><div id="teste"></div></td></tr>
				</table>
			</td>
		</tr>
...
<%				ElseIf categoria = CAT_EQUIPAMENTO Then %>
					Plataforma<br>
					<%call comboServicosPlataformas("plataforma", conn, "N", "P")%>
...
<%
				if categoria <> "" then
					...
					If bln_AcessoRAT Then
						ssql = "select f.fab_id as VALOR, f.fab_nome as DESCRICAO from sce_fabricantes f WHERE fab_id = " & valor
						Set rec = conn.execute(ssql) %>

						<input type="hidden" name="fab_id" class="form" value="<%=valor%>">
						<input type="text" name="fab_desc" class="form" size="50" value="<%If Not rec.Eof Then Response.Write rec("descricao")%>" readonly>

<%				Else %>
					<!--<select name="fab_id" class="form" style="width:300" onchange="javascript:AlteraCombos();">-->
					<select name="fab_id" class="form" style="width:300;" onChange="javascript:buscaModelos(this.value);">
...
<%
					...
					If bln_AcessoRAT Then
						...
						<input type="hidden" name="mod_id" class="form" value="<%=valor%>">
						<input type="text" name="mod_desc" class="form" size="50" value="<%If Not rec.Eof Then Response.Write rec("descricao")%>" readonly>
<%					Else %>

					<select name="mod_id" id="mod_id" class="form" style="width:280;" onChange="javascript:buscaPN(this.value);">
...
%>
			...
			<td class="texto">
					PN:<br><input type="Text" value="" name="txtPN" id="txtPN" class="texto" readonly="true">
			</td>
    		</tr>
...
<%			end if %>
		...
		<tr class="texto">
			<td>
				Localização<br>
				<input type="text" name="localizacao" class="form" size="50" maxlength="255" value="<%=localizacao%>">
				</td>
<%			if categoria = CAT_EQUIPAMENTO then %>
				<td colspan="2">
				Conforme<br>
...
<%			end if %>
		...
		<tr>
				...
				<textarea class="form" name="obs" cols="140" rows="9"><%=obs%></textarea>
			</td>
		</tr>
...
<%'-- Estes dados sao de equipamentos que são INSTRUMENTAIS
		...
		<tr>
				...
				<textarea class="form" name="manutencaopreventiva" cols="140" rows="9"><%=manutencaopreventiva%></textarea>
			</td>
		</tr>
		<tr><td colspan="3">&nbsp;</td></tr>
		<tr>
			<td colspan="3">
				...
				<tr>
					<td class="texto" width="40%">
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						Frequencia de Calibração: <input type="text" class="texto" name="freq_calibracao" maxlength="7" size="5" value="<%=freq_calibracao%>">
					</td>
				</tr>
...
-->
<%			end if	'categoria = CAT_EQUIPAMENTO%>
...
<%	if categoria = CAT_EQUIPAMENTO then %>
	<tr><td class="titulo" colspan="3">Acessórios do Equipamento</td></tr>
	<tr>
		<td colspan="3">
...
<%
	...
	<tr>	
		<td class="texto" colspan="3">Propriedade:<br>
<%If bln_AcessoRAT Then
...
%>
	var meuForm = document.all;
		...
		meuForm.categoria.readOnly = true;
