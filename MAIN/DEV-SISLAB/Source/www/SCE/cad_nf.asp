...
call ImprimeCabecalho ("", MENU_ON, true, titulo, "", "history.go(-1);")
    		  ...
    		  <tr>
				<td>
					<TABLE WIDTH="100%" cellpadding="0" cellspacing="0" class="texto">
							<TR>
								<TD>Número da Nota:&nbsp;
								<input type="text" class="form" name="nf_numeronota" size="10" maxlength="30" value="<%=nf_numeronota%>">
								</td>
								<td><input type="Checkbox" name="devolucao" value="1" <%if devolucao then response.write "checked"%>>&nbsp;Devolucao Completa</td>
								<TD>Qtde. Volumes:&nbsp;
								<input type="text" class="form" name="nf_qtdevolumes" size="10" maxlength="30"value="<%=nf_qtdevolumes%>">
								</td>
								<TD>Valor Total&nbsp;R$:
								<input type="text" class="form" name="nf_valortotal" size="15" maxlength="30" value="<%=nf_valortotal%>"></td>
							</tr>
						</table>
					</td>
				</tr>
					...
					<td>
						<TABLE WIDTH="100%" cellpadding="0" cellspacing="0" class="texto">
							<TR>
								...
								<input type="text" class="form" name="nf_nconhecimento" size="10" maxlength="30" value="<%=nf_nconhecimento%>"></td>
								<TD>
									Tipo:&nbsp;
									<select name="nf_tipo" class="form" onChange="javascript:tipoNota(this);">
...
<%			if trans_id = "" then valor = "" else valor = cstr(trans_id)
			call comboFornecedor("trans_id", conn, valor, "N", "TRANSPORTADORA", false)%>
					</td>
				</tr>
...
<%			if enf_id = "" then valor = "" else valor = cstr(enf_id)
			call comboFornecedor("enf_id", conn, valor, "N", "FORNECEDOR", false)%>
					</td>
				</tr>
				...
				<tr>
					<td>
						<TABLE WIDTH="600" cellpadding="0" cellspacing="0" class="texto">
							<tr>
								<td><br>Natureza de Operação:&nbsp;
<%			if no_id = "" then valor = "" else valor = cstr(no_id)
			call comboBDSQL( "no_id", conn, "select no_id as VALOR, no_descricao as DESCRICAO from sce_natureza_operacao order by no_descricao", valor, "N")%>
								<script language="JavaScript">
...
<%						if nf_id_pai = "" then valor = "" else valor = cstr(nf_id_pai)
						call comboBDSQL( "nf_id_pai", conn, "select a.nf_id as VALOR, b.enf_nome + ' (NF ' + CAST(a.nf_numeronota as VARCHAR) + ')' as DESCRICAO from sce_nota_fiscal a, sce_empresa_nota_fiscal b where a.enf_id = b.enf_id order by b.enf_nome, a.nf_numeronota", valor, "N")%>
					</td>
				</tr>
				...
				<tr id="tr_aceitacao" style="display: block;">
					<td>
						<TABLE WIDTH="600" cellpadding="0" cellspacing="0" class="texto">
							<TR>
								<TD colspan=3>
								<span class=titulo>Aceitação:</span><br>
								Integridade:&nbsp;
	...
	<tr>
		<td align="right">
			<input class="form" type="submit" <%if nf_id = "" then%> value="Cadastrar" <%else%> value="Alterar" <%end if%>>
<%
if nf_id <> "" then%>
			&nbsp;&nbsp;
			<input class="form" type="button" value="Nova Nota" onClick="javascript:location.href='cad_nf.asp';">
			&nbsp;&nbsp;
			<input class="form" type="button" value=" Excluir " onClick="javascript:excluirNota();"><%
end if
%>
		</td>
	</tr>
