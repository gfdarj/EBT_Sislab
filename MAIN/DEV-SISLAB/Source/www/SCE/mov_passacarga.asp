...
<%
call ImprimeCabecalho ("", MENU_ON, true, "Movimentação - Passagem de Carga", "", "history.go(-1);")
%>
...
<tr>
	<th>
		Passagem de carga solicitada com sucesso. <br>Aguarde a confirma&ccedil;&atilde;o
...
<tr>
	<td><span class="titulo">Meus agendamentos: (<i><%=Replace(Ucase(Request.ServerVariables("REMOTE_USER")), "EMBRATEL\", "")%></i>)</span><br>
		<%call comboMeusAgendamentos(true, "txtAS_origem", "ag_numero_origem", conn, cstr(ag_numero), "N")%>
...
<tr>
	<td>
		<b>*&nbsp;</b>Equipamentos em condi&ccedil;&otilde;es de carga (sem reservas no per&iacute;odo do agendamento)<br>
		<select multiple name="eq_origem" style="width: 640px;" class="form" size="6"></select>
	</td>
</tr>
<tr><td>&nbsp;</td></tr>
<tr>
	<td><span class="titulo">Selecione o agendamento de <i><span style="color: red;">destino</span></i>:</span><br>
		<%'call comboMeusAgendamentos(false, "txtAS_destino", "ag_numero_destino", conn, cstr(ag_numero), "N")%>
		<%call comboAgendamento("txtAS_destino", "ag_numero_destino", conn, cstr(ag_numero), "N")%>
		&nbsp;&nbsp;&nbsp;
		<button class="form" onclick="javascript:passarEqDestino();"><img src="img/seta_abaixo.gif"> Mover para destino</button>
...
<tr><td>&nbsp;</td></tr>
<tr>
	<td align="center"><button class="form" onclick="javascript:validaPassagem();">&gt;&gt; Passar a carga dos itens selecionados</button></td>
</tr>
</table>
</form>
<script language="JavaScript">
