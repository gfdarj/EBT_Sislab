...
%>
<table width="100%" class="texto" cellpadding="2" cellspacing="0" border="0">
<tr>
	<td><p class="titulo">Listagem de Notas Fiscais<%=titulo%></p></td>
	<td align="right" class="texto"><%=url_xls%></td>
</tr>
</table>
<%
if not rec.eof then
%>
<table width="100%" class="texto" cellpadding="2" cellpadding="0"><tr><td><i>As notas em destaque (<span class="vencido">&nbsp;&nbsp;</span>) est&atilde;o com vencidas.</i></td></tr></table>
<table width="100%" class="texto" cellpadding="2" cellspacing="0" border="1">
	<tr>
		<td class="titulo" >Fornecedor</td>
		<td class="titulo"  align="center">Data de Emissão</td>
		<td class="titulo" align="center">Data de Receb. / Expedição</td>
		<td class="titulo" align="center">Número /<BR>Valor</td>
		<td class="titulo" >Natureza Opera&ccedil;&atilde;o</td>
		<td class="titulo"  align="center">Tipo</td>
		<td class="titulo"  align="center">Validade (dias)</td>
		<td class="titulo"  align="center">Data de Venc.</td>
		<td class="titulo" align="center">NF Aquis.</td>
		<td class="titulo" align="center">Dev. Tot.</td>
    </tr>
<%	cont = 0
	while (not rec.eof)
		cont = cont+1
...
<!--	<tr <%'if bg = 0 then%>bgcolor="#C0E0EF"<%'end if%>> -->
	<tr <%if not IsNull(rec("nf_datavencimento_M")) then if cdate(rec("nf_datavencimento_M")) < date() and (rec("nf_devolucaocompleta_M") = 0) then response.write "class='vencido'" else if bg = 0 then response.write "bgcolor='#C0E0EF'" end if end if else if bg = 0 then response.write "bgcolor='#C0E0EF'" end if %>>
		<td class="texto"><%=ConverteNuloHTML(rec("enf_nome_M"))%>&nbsp;</td>
		<td class="texto" align="center"><%=ConverteNuloHTML(rec("nf_dataemissao_M"))%></td>
		<td class="texto" align="center"><%=ConverteNuloHTML(rec("nf_recebimento_M"))%></td>
...
<%			End If %>
		...
		<td class="texto"><%=ConverteNuloHTML(rec("NO_DESCRICAO_M"))%></td>
		<td class="texto" align="center"><%=Left(ConverteNuloHTML(rec("NF_TIPONOTA_M")), 1)%></td>
		<td class="texto" align="center"><%=ConverteNuloHTML(rec("nf_validade_M"))%></td>
		<td class="texto" align="center"><%=ConverteNuloHTML(rec("nf_datavencimento_M"))%></td>
		<td align="center">
<%			Response.Write ConverteNuloHTML(rec("nf_numeronota_PAI_M"))%>
		</td>
		<td class="texto" align="center">
<%
...
<p class="texto" align="right"><i>Registros encontrados: <%=rec.recordcount%></i></p>
<%
else%>
	<p class="titulo" align="center"><i>Nenhuma Nota Fiscal encontrada</i></p><%
end if%>
<br>
<%
conn.close
