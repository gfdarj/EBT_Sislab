...
<b>Comentários do Agendamento <%=auxselag%><br>
Item&nbsp;
<%if auxselit=1 then%>
Comunicação
<%end if%>
...
<%
sSQL = "Select * From [PesquisaSatisfacao] WHERE (psq_nag=" &auxselag& "); "
...
if not(((auxselit=1) and IsNull(objSiteRS("PSQ_C1"))) or _
...
((auxselit=11) and IsNulo(objSiteRS("PSQ_C11")))) then
...
<%end if%>
<%if auxselit=5 then%>
<%=retornaopcao1(objSiteRS("PSQ_R5"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R5"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C5")%>
<%end if%>
<%if auxselit=6 then%>
<%=retornaopcao1(objSiteRS("PSQ_R6"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R6"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C6")%>
<%end if%>
<%if auxselit=7 then%>
<%=retornaopcao1(objSiteRS("PSQ_R7"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R7"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C7")%>
<%end if%>
<%if auxselit=8 then%>
<%=retornaopcao1(objSiteRS("PSQ_R8"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R8"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C8")%>
<%end if%>
<%if auxselit=9 then%>
<%=retornaopcao1(objSiteRS("PSQ_R9"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R9"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C9")%>
<%end if%>
<%if auxselit=10 then%>
<%=retornaopcao1(objSiteRS("PSQ_R10"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R10"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C10")%>
<%end if%>
<%if auxselit=11 then%>
<%=retornaopcao1(objSiteRS("PSQ_R11"))%>&nbsp;(<%=retornaopcao(objSiteRS("PSQ_R11"))%>)&nbsp;&nbsp;<%=objSiteRS("PSQ_C11")%>
<%end if%>
...
<td bgcolor="#EEEEEE" align="right"><font face="tahoma" style="font-size=9pt" color="#222222">
<b>nome: <%=objSiteRS("PSQ_nome")%>&nbsp;- <%=objSiteRS("PSQ_orgaoEmpresa")%></b></font>
</td>
<td bgcolor="#EEEEEE">
</td>
</tr>
