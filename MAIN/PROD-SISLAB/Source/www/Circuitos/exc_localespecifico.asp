...
call Env.StoredProcedure( true, objSP, "SP_FAC_EXCLUI_LOCAL_ESPECIFICO")
with( objSP )
	.Parameters.Append( .CreateParameter( "RETORNO", adInteger, adParamReturnValue ) )
	.Parameters.Append( .CreateParameter( "@leeID", adInteger, adParamInput, , request.form("locais") ) )
	.execute()
	ret = .Parameters( "RETORNO" )
end with
call StoredProcedure( false, objSP, "SP_FAC_EXCLUI_LOCAL_ESPECIFICO")
...
if ret > 0 then
	...
	window.parent.AtualizaLocais(str);<%
		...
		alert("Ocorreu um erro ao apagar este local. (<%=ret%>)");<%
end if
%>
</script>
