...
If ehNovo Then
	Call ImprimeCabecalho("", MENU_ON, True, "Cadastro de Reserva de Equipamentos", "", "history.go(-1);")
Else
	Call ImprimeCabecalho("", MENU_ON, True, "Altera Reserva de Equipamentos", "", "history.go(-1);")
End If
%>
...
%>
<tr>
	<td colspan="2" valign="top"><span class="titulo">AS&nbsp;</span>
...
<tr>
	<td colspan="2">
		<table width="100%" class="texto" border="0" cellpadding="0" cellspacing="0">
		<tr>
			<td valign="top">
				Ambiente(s):<br>
			...
			<td valign="top">Respons&aacute;vel T&eacute;cnico<br>
				<% Call comboBDSQL("ag_responsavel", conn, "select upper(USERID) as VALOR, CAST(NOME as VARCHAR(40)) as DESCRICAO from USERCRT where USERID = '" & responsavel & "' order by Nome", responsavel, "")%>
			</td>
			<td valign="top" align="left">
				Inicio:<br><input type="Text" name="ag_datainicio" class="form" size="12" value="<%=dt_inicio%>" disabled>&nbsp;
			</td>
			<td valign="top" align="left">
				Fim:<BR><input type="Text" name="ag_datatermino" class="form" size="12" value="<%=dt_termino%>" disabled>
			</td>
		</tr>
		</table>
	</td>
...
<tr>
	<td colspan="2">
		<span class="titulo">Filtros:</span>&nbsp;&nbsp;&nbsp;(Selecione os campos abaixo para filtrar os dados do equipamento)
		<table class="texto" width="100%" border="0">
			<tr>
				<td>Modelo:</td>
				<td><input type="Text" name="modelo" class="form" size="50" on></td>

				<td>Descri&ccedil;&atilde;o:</td>
				<td><input type="Text" name="desc_modelo" class="form" size="50"></td>
			</tr>
			<tr>
				<td>Fabricante:&nbsp;</td><td><%call comboBDSQL("fabricante", conn, "select f.fab_id as VALOR, f.fab_nome as DESCRICAO from sce_fabricantes f order by f.fab_nome", "", "N")%></td>
				<td>Cod.Barras:</td>
				<td>
					<input type="Text" name="coditem" class="form" size="25">
					&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="Button" value="Filtrar &gt;&gt;" class="form" onClick="javascript:filtraItem();">
				</td>
			</tr>
			<tr>
				<td>Nota Fiscal:</td>
				<td colspan="3">
					<input type="Text" name="notafiscal" size="6" class="form">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<%Call ComboSituacaoEq("status", true, true, true, STATUS_EM_ESTOQUE)%>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<tr>
				<td>Documento:</td>
				<td colspan="3">
					<input type="Text" name="documento" size="6" class="form">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<tr>
				<td>Fornecedor:</td>
				<td colspan="3">
				<%call comboFornecedor("fornecedor", conn, "", "N", "FORNECEDOR", false)%>
				&nbsp;
				</td>
			</tr>
		</table>
...
<tr>
	<td colspan="2">
		<span class="titulo">Itens Reservados:</span>
		&nbsp;&nbsp;&nbsp;&nbsp;
		(qtde:&nbsp;<input type="Text" name="qtd_item" value="1" size="3" class="form">&nbsp;
		<a href="#" onClick="javascript:InsereItem(document.all.qtd_item.value);">novo item</a>)
		<table width="100%" cellpadding="0" cellspacing="0" class="texto" border="1" id="tb_reserva" style="border: thin solid silver;">
		<tr>
...
if ag_numero <> "" then
	...
	if not (rec.eof and rec.bof) then
		...
		while not rec.eof
			...
			reservado = VerificaReservaItem(true, Conn, ag_numero, rec("EQ_ID"), false)
%>		<tr id="linha_<%=linha%>">
			...
			<td id="linha_<%=linha%>_col_3" valign="top"><script language="JavaScript">inputText(true, 'dt_ini_<%=linha%>', '<%=rec("REQ_DATAINICIO")%>', 13, 10, 'onKeyPress="formataData(this);" onKeyUp="proxCampo(this, document.all.dt_fim_<%=linha%>);"');</script></td>
			<td id="linha_<%=linha%>_col_4" valign="top"><script language="JavaScript">inputText(true, 'dt_fim_<%=linha%>', '<%=rec("REQ_DATATERMINO")%>', 13, 10, 'onKeyPress="formataData(this);" onKeyUp="proxCampo(this, document.all.cmb_setup_<%=linha%>);"');</script></td>
			<td id="linha_<%=linha%>_col_5" align="center" valign="top"><script language="JavaScript">comboAmostraEq(true, 'cmb_setup_<%=linha%>', '<%=rec("REQ_EQSETUP")%>');</script></td>
			<td id="linha_<%=linha%>_col_6" align="center" valign="top"><script language="JavaScript">comboAmbiente(true, 'cmb_amb_<%=linha%>', '<%=rec("AMB_ID")%>');</script></td>
...
<%			if (rec("REQ_ACEITO")) and (session("status") = PERFIL_RAT) or rec("REQ_MOVIMENTOU") or (not ehAgValido)then%>
...
<%			Call MarcaItemREservado((reservado <> "") , linha)
			rec.MoveNext
			linha = linha + 1
		wend%>
...
<tr><td colspan="2">&nbsp;</td></tr>
<tr>
	<td colspan="2"><span class="titulo">Observações:</span><br>
		<textarea name="obs" class="form" rows="10" cols="110"><%=obs%></textarea>
	</td>
</tr>
<tr><td colspan="2">&nbsp;</td></tr>
</table>
...
<tr>
	<td colspan="2" align="right">
	  	<input type="button" name="Submit" value="<%if ehNovo then response.write "Cadastrar Reserva" else response.write "Alterar Reserva"%>" class="form" onClick="javascript:cadastraReserva();">&nbsp;&nbsp;
<%'-- se for um equipamneto ja cadastrado entao exibo o botao excluir
if not ehNovo then%>
	  	<input type="button" name="btnNovo" value="Nova Reserva" class="form" onClick="javascript:location.href='cad_reserva.asp';">&nbsp;&nbsp;
	  	<input type="button" name="btnExcluir" value="Excluir Reserva" class="form" onClick="javascript:excluirReserva();">&nbsp;&nbsp;
		<script language="JavaScript">
...
<%
if (not ehAgValido) and (session("status") = PERFIL_RAT) then
%>
