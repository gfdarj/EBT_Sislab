...
s =	"SELECT DISTINCT e.EQ_ID, e.EQ_CODIGOBARRAS, e.MOD_CODNOME, e.MOD_DESCRICAO, " & _
...
s = s & "from vw_SCE_Equipamentos_Fabricantes e "
w = ""
...
set objRS = conn.execute( s )

call ImprimeCabecalho("Reserva - Filtro de equipamento", MENU_OFF, false, "Filtro de Items para Reserva", "", "window.close();")
%>
...
<tr>
	<td>
		...
		<table class="texto" width="100%" border="1" cellpadding="2" cellspacing="0" style="border: thin silver solid;">
		<tr valign="top">
...
if objRS.eof and objRS.Bof then%>
		<tr><td colspan="9<%'if request("notafiscal") <> "" or request("fornecedor") <> "" then response.write "10" else response.write "9"%>" align="center" class="titulo">Nenhum equipamento encontrado !</td></tr><%
else
	linha = 1
	while not objRS.eof
		reservado = Trim(VerificaReservaItem(false, conn, request("ag_numero"), objRS("EQ_ID"), true))%>
...
<!--			<td id="<%'="linha_"&linha&"_col_5"%>"><%'if isNull(objRS("EQ_LOCALIZACAO")) then response.write "&nbsp;" else response.write objRS("EQ_LOCALIZACAO")%></td>-->
			...
			<td id="<%="linha_"&linha&"_col_7"%>" align="center">
				<%=PegaStatusItem(Conn, objRS("EQ_ID"), true)%>
			</td>
<!--			<td id="<%'="linha_"&linha&"_col_8"%>" align="center"><%'if isNull(objRS("EQ_INSTRUMENTAL")) then response.write "&nbsp;" else response.write Bool2Str(objRS("EQ_INSTRUMENTAL"))%></td>-->
			<td id="<%="linha_"&linha&"_col_8"%>" align="center"><%if isNull(objRS("EQ_CONFORME")) then response.write "&nbsp;" else response.write Bool2Str(objRS("EQ_CONFORME"))%></td>

<%
listaNF = PegaNotaFiscalItem(Conn, objRS("EQ_ID"))
%>
			<td id="<%="linha_"&linha&"_col_9"%>"><%if listaNF = "" then response.write "&nbsp;" else response.write listaNF%></td>
		</tr>
<%		if reservado <> "&nbsp;" then%>
		<tr style="color: red;">
			<td>&nbsp;</td>
			<td colspan="7"><b>Reservas AG:</b> <%response.write replace(replace(reservado, " ", ""), ",", "<br>")%></td>
		</tr>
...
<tr>
	<td align="center">
		<input type="Button" value=" Incluir " class="form" onClick="javascript:IncluiListaItens();">
		&nbsp;&nbsp;&nbsp;
		<input type="Button" value=" Voltar " class="form" onClick="javascript:window.close();">
	</td>
</tr>
</table>
<script language="JavaScript">
