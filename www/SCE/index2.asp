...
Sub ImprimeControlesSaidaCalibracao()
	...
	sTitulo = "Controle das Próximas Calibrações (90 dias)"

	sSql = _
			"SELECT E.EQ_CODIGOBARRAS AS EQ_CODIGOBARRAS_M, E.EQ_LOCALIZACAO AS EQ_LOCALIZACAO_M, M.CDE AS CDE_M, " & _
			...
			"WHERE m.MOV_DATA + ISNULL(e.EQ_FREQ_CALIBRACAO, 0) <= GETDATE() + 90 " & _
...
<%			RS.MoveNext
		WEnd%>
							<tr><td colspan="2" align="right"><i>Total de itens: <%=RS.RecordCount%></i></td></tr>
<%	Else
...
Sub ImprimeControleSaidaParaManutencao()
	...
	sTitulo = "Pendencias de retorno Manut./Calibração"

	sSql = _
			"SELECT E.EQ_CODIGOBARRAS AS EQ_CODIGOBARRAS_M, M.CDE AS CDE_M, CONVERT(VARCHAR, M.MOV_DATA, 103) AS DT_SAIDA_M, " & _
			...
			"WHERE M.CDE IS NOT NULL AND LTRIM(RTRIM(M.CDE)) <> '' AND LTRIM(RTRIM(M.CDE)) <> ' ' AND (M.TIPO = " & MOV_EXPEDICAO & ")"
	Set RS = conn.Execute(sSql)
...
Sub ImprimeControles(tipo, chr_Titulo1, chr_Titulo2)
	...
	If (tipo = CONTROLE_CALIBRACAO) Or (tipo = CONTROLE_MANUTENCAO) Then
		s =	"SELECT EQ_CODIGOBARRAS AS [Código Barras_M], MOD_CODNOME AS [Modelo_M], CONVERT(varchar, EQC_VENCIMENTO, 103) AS [Vencimento_M], EQC_TIPO AS [Tipo] " & _
	...
	Else
		s =	"SELECT EQ_CODIGOBARRAS  AS [Código Barras_M], MOD_CODNOME AS [Modelo_M], DESC_STATUS AS [Status_M] /*CASE WHEN LEN(MOD_CODNOME)>10 THEN LEFT(MOD_CODNOME, 7) + '...' ELSE MOD_CODNOME END*/ " & _
...
<%		while not rec.eof%>
							<tr><td><%=ImprimeTipoManutencaoHTML(rec("TIPO"))%><%=ConverteNuloHTML(rec(0))%></td><td><%=ConverteNuloHTML(rec(2))%></td></tr>
<%			rec.moveNext
		wend%>
...
Sub ImprimeControles2(chr_Titulo1, chr_Titulo2, chr_Titulo3, chr_Titulo4)
	...
	s =	"SELECT EQ_CODIGOBARRAS AS [Código Barras_M], MOD_CODNOME AS [Modelo_M], CASE WHEN STATUS = 0 THEN 'Cadastrado' ELSE DESC_STATUS END AS [Status_M], STATUS AS [Cod. Status], EQ_LOCALIZACAO AS EQ_LOCALIZACAO_M " & _
		"FROM vw_SCE_Equipamentos_Fabricantes " & _
		"WHERE STATUS <> " & STATUS_EXPEDIDO & _
		" AND STATUS <> " & STATUS_EXPEDIDO_SUBST & _
		" AND EQ_CONFORME = 0"
...
<%		while not rec.eof%>
							<tr>
								...
								<td><%=ConverteNuloHTML(IIf(Len(rec(1)) > 10 , Left(rec(1), 10) & "...", rec(1)))%></td>
								<td><%
...
'vinculacao de equipamento com a nota é pela movimentacao - entrada logistica
	...
	url_xls = "<div align='right'><a href=""../excel.asp?titulo=" & Titulo & "&sql=" & Server.UrlEncode(s_xls) & """ target='_blank' alt='Exporta esta listagem para o Excel'><font color='#008000'><b>XLS</b></font></a></div>"%>
				...
				<tr>
					<td>
						<table width="100%" class="texto" border="0" cellpadding="0" cellspacing="2">
						<tr>
...
%>
							<tr>
								<td colspan="8" align="right">
									...
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<i>Total de itens: <%=rec.recordcount%></i>
								</td>
							</tr>
<%	else
