...
'response.write s_orig & "<BR>"
'response.write s_dest
%>
<p class="titulo">Listagem de Passagem de Carga</p>

<p class="titulo">Cargas Passadas <%=ImprimePassagemAS(request("ag_numero"))%></p>
<%
Set rec = Conn.Execute(s_orig)
call ImprimeListaPassagem(conn, rec, "ORIG", request("ag_numero"))
rec.Close

if request("ag_numero") <> "" or request("ag_responsavel") <> "" then%>
	<p class="titulo">Cargas Recebidas <%=ImprimePassagemAS(request("ag_numero"))%></p>
<%
	Set rec = Conn.Execute(s_dest)
...
Function ImprimePassagemAS(ag)
	if trim(ag) <> "" then ImprimePassagemAS = "<span class='titulo'>(AS " & ag & ")</span>"
End Function
...
<%	else%>
<p align="center" class="titulo"><i>Nenhuma carga encontrada</i></p>
<%	end if
End Sub
%>
